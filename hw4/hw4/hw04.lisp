...
(check= (lobv5p *secret-message*) t)

"Property 1"
(property ()
          (== t (lobv5p *secret-message*)))
...
(definec next-bv-h(b :bv carry :bool) :bv
  (cond
   ((lendp b) '())
   (t (cond
        ((== carry nil) (append (list (first b))(next-bv-h (rest b) nil)))
        ((and (== carry t) (== (first b) t)) (append (list nil) (next-bv-h (rest b) t)))
        (t (append (list t) (next-bv-h (rest b) nil)))))))
(check= (next-bv-h '(t nil) t) '(nil t)) 
(check= (next-bv-h '(nil nil) t) '(t nil)) 
(check= (next-bv-h '(t t t t t) t) '(nil nil nil nil nil))
(check= (next-bv-h '(t t nil nil t t) t) '(nil nil t nil t t))
(check= (next-bv-h '(nil nil) nil) '(nil nil))

(definec next-bv (b :bv) :bv
  (next-bv-h b t))

(check= (next-bv '(t nil)) '(nil t)) 
(check= (next-bv '(nil nil)) '(t nil)) 
(check= (next-bv '(t t t t t)) '(nil nil nil nil nil))
(check= (next-bv '(t t nil nil t t)) '(nil nil t nil t t))
...
(definec generate-char-bvn-range-aux (v :bv n :nat) :lobv
  (if (== n 0) '() (append (list v) (generate-char-bvn-range-aux (next-bv v) (- n 1)))))

(check= (generate-char-bvn-range-aux '(nil nil) 2) '((nil nil)(t nil)))
(check= (generate-char-bvn-range-aux '(nil nil) 3) '((nil nil)(t nil)(nil t)))
(check= (generate-char-bvn-range-aux '(nil nil) 4) '((nil nil)(t nil)(nil t)(t t)))
(check= (generate-char-bvn-range-aux '() 3) '(()()()))
(check= (generate-char-bvn-range-aux '(nil nil) 0) '())
(property (v :bv n :nat)
          :proofs? nil
          :hyps (== n 0)
          (== (generate-char-bvn-range-aux v n) '()))

(property (v :bv n :nat)
          :proofs? nil
          :hyps (> n 0)
          (== (first (generate-char-bvn-range-aux v n)) v))

;; Question 5
;; Define the function n-copies, as specified above. 
(definec n-copies (x :all n :nat) :tl
  (if (== n 0) '() (append (list x) (n-copies x (- n 1)))))

(check= (n-copies 1 2) '(1 1))
(check= (n-copies 1 0) '())
(check= (n-copies nil 5) '(nil nil nil nil nil))
...
(definec generate-char-bvn-range (n :nat) :lobv
  (generate-char-bvn-range-aux (n-copies nil n) (expt 2 n)))
...
(check= (generate-char-bvn-range 3)
        '((NIL NIL NIL)
          ...
          (NIL T T)
          (T T T)))
(check=(generate-char-bvn-range 0) '(nil))
(property (n :nat)
          (== (first (generate-char-bvn-range n)) (n-copies nil n)))
(property (n :nat)
          (== (length (generate-char-bvn-range n)) (expt 2 n)))
...
(definec list-zip (x :tl y :tl) :tl
  :pre (= (len x) (len y))
  (if (lendp x) '() (append (list (cons (first x) (first y))) (list-zip (rest x)(rest y)))))
...
(definec find-car (a :all l :alist) :list
  (cond 
   ((lendp l) nil)
   (t (if (== (car (car l)) a) (car l) (find-car a (rest l))))))

(check= (find-car #\W *bv5-char-map*) '(#\W nil t t nil t))
(check= (find-car 123 '((1 2 3)(3 4 5))) nil)
...
(definec find-cdr (a :all l :alist) :list
  (cond 
   ((lendp l) nil)
   (t (if (== (cdr (car l)) a) (car l) (find-cdr a (rest l))))))

(check= (find-cdr '(nil t t nil t) *bv5-char-map*)
        '(#\W nil t t nil t))
(check= (find-cdr 123 '((1 2 3)(3 4 5))) nil)
...
(definec xor-bv (b1 :bv b2 :bv) :bv
  ...
  (cond 
   ((lendp  b1) '())
   (t (append (list (xor (first b1) (first b2))) (xor-bv (rest b1) (rest b2))))))

(check= (xor-bv '(t t t) '(t t t)) '(nil nil nil))
(check= (xor-bv '(nil t t) '(t t t)) '(t nil nil))
(check= (xor-bv '() '()) '())
...
(definec encrypt (m :lochar s :lobv5) :lobv
  :ic (<= (len m) (len s))
  (cond 
   ((lendp m) '())
   (t (append (list (encrypt-char (first m) (first s))) (encrypt (rest m) (rest s))))))
...
(check= (decrypt-bv5 '(t t t t t) '(t t t t t)) #\A)
...
(definec decrypt (e :lobv5 s :lobv5) :lochar
  ...
  (cond 
   ((lendp e) '())
   (t (append (list (decrypt-bv5 (first e) (first s))) (decrypt (rest e) (rest s))))))
...
(definec game-over-h (m :lobv5 keys :lobv5) :lostring
  (cond 
   ((lendp keys) '())
   (t (append (list (coerce (decrypt m (n-copies (first keys) (len m))) 'string)) (game-over-h m (rest keys))))))

(definec game-over () :lostring
  (game-over-h *secret-message* (generate-char-bvn-range 5)))#|ACL2s-ToDo-Line|#
